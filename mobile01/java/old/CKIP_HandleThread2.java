 ...
 * @author FleshWhiter
 */
public class CKIP_HandleThread2 extends Thread {
        ...
        CKIP_HandleThread2(int start, int end){
        	File_start = start;
        	File_end = end;
        }
        ...
        public static void main(String[] args) throws IOException {
            ...
            CKIP_HandleThread2 ckip = new CKIP_HandleThread2(File_start, File_end);
            ckip.start();
        ...
        public void run(){
			...
			if(dict.isDirectory())
			{
				String[] filenames = dict.list();
				if(filenames.length < File_end)
					File_end = filenames.length;
        ...
        public static void Rewrite(String f_name) {
			try {
				FileInputStream f_source = new FileInputStream(new File("data/iphone_data/"+f_name));
				
				File new_f = new File("w_data/iphone_w_data/w_"+f_name);
				if(!new_f.exists()){
					System.out.println(new_f+"not Exists!");
				}
				else
				{
					FileWriter f_rewrite = new FileWriter(new_f);
					try (BufferedWriter brw = new BufferedWriter(f_rewrite)) {
						...
						System.out.println(f_name+"\t"+new Date());
